...
```{r}
thirty_or_younger <- Trips_during_school_year %>%
...
```{r}
older_than_thirty <- Trips_during_school_year %>% 
...
```{r}
Top_50_Stations <- Popular_Stations %>% head(50) 
leaflet(Top_50_Stations) %>% 
  addTiles() %>%
  addMarkers(lng = ~avg_lon, lat = ~avg_lat, popup = ~name
             )
```
